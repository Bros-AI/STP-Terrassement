...
<!-- critical:start --> / <!-- critical:end --> markers on every ROOT template
page, switching css/styles.css to the async preload+onload pattern there.
Blog articles keep the plain blocking stylesheet (their above-fold is text
and they carry their own inline article styles).
...
def main():
    ...
    async_css = ('    <link rel="preload" href="css/styles.css" as="style" '
    ...
    print(f'critical CSS installed/refreshed on {done} root pages')
